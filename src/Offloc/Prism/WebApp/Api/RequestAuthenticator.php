...
use Offloc\Prism\Domain\Model\Service\ServiceRepositoryInterface;
...
class RequestAuthenticator
{
     ...
     *
     * @return \Offloc\Prism\Domain\Model\Service\Service
     */
    public function authenticate(Request $request)
    {
        $authorization = array();

        if (!preg_match('/^OfflocPrism (.+)$/', $request->headers->get('authorization'), $matches)) {
        ...
        foreach ($headers as $key => $value) {
            if (
                ...
                'host' === strtolower($key) ||
                'x-offloc-prism-' === substr(strtolower($key), 0, 16)
            ) {
                $signedItems[] = $value[0];
            }
        }
