...
class Admin extends AbstractApp
{
    protected function configure()
    {
        ...
        $app['offloc.router.admin.users'] = $app['offloc.router.projectRoot'].'/config/admin.users.json';
        ...
        $app['security.firewalls'] = array(
            ...
            'secured' => array(
                ...
                'users' => $app->share(function($app) {
                    $adminUsersFile = $app['offloc.router.admin.users'];
        ...
        $app->post('/search', function(Request $request) use ($app) {
            ...
            return $app->redirect($app['url_generator']->generate(
                'offloc_router_admin_route_detail',
                array('routeId' => $route->id(),)
            ));
        })->bind('offloc_router_admin_search');

        $app->get('/services', function() use ($app) {
            return $app['twig']->render('service_list.html.twig', array(
                'services' => $app['offloc.router.domain.model.service.serviceRepository']->findAll(),
            ));
        })->bind('offloc_router_admin_service_root');

        $app->post('/services', function(Request $request) use ($app) {
            $serviceFactory = $app['offloc.router.domain.model.service.serviceFactory'];
            $serviceRepository = $app['offloc.router.domain.model.service.serviceRepository'];
            $session = $app['offloc.router.domain.model.session'];
            ...
            return $app->redirect($app['url_generator']->generate(
                'offloc_router_admin_service_detail',
                array('serviceKey' => $service->key(),))
            );
        })->bind('offloc_router_admin_service_create');
        ...
        })->bind('offloc_router_admin_service_detail');
        ...
        })->bind('offloc_router_admin_service_update');
        ...
        })->bind('offloc_router_admin_route_create');
        ...
        })->bind('offloc_router_admin_route_detail');
        ...
        })->bind('offloc_router_admin_login');
    }
}
