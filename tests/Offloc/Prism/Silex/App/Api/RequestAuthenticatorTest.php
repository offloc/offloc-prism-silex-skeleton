...
{
    protected function createMockServiceRepository(\Offloc\Prism\Domain\Model\Service\Service $service)
    {
        $serviceRepository = $this
            ->getMockBuilder('Offloc\Prism\Domain\Model\Service\ServiceRepositoryInterface')
    ...
    protected function makeAuthenticateRequest($service, $body, $host, $verb, $contentType, $resource, $time, $queryString, $additionalHeaders, $tweakServerCallback = null)
    {
        ...
        foreach ($server as $key => $value) {
            ...
            if (
                ...
                'http_x_offloc_prism_' === substr(strtolower($key), 0, 21)
            ) {
                $signedItems[] = $value;
            }
        }
        ...
        $server['HTTP_AUTHORIZATION'] = 'OfflocPrism Credential=' . $service->key(). ", Signature=" . String::hexToBase64(hash_hmac('sha256', $stringToSign, $service->secret()));
    ...
    public function testAuthenticateSuccess()
    {
        ...
        $testService = new \Offloc\Prism\Domain\Model\Service\Service(
            'serviceKey',
            'Some Service',
            'http://service.com'
        );
